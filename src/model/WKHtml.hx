...
class WKHtml
{
	...
	@:isVar public var format(get, set) : String;
	...
	public function renderUrl(path : String) : String
	{
		...
		if(ok)
		{
			var result = sys.io.File.getContent(out);
	...
	{
		return (~/(\n\r|\n|\r)/gm).split(err).map(function(line) {
	...
	{
		var process = new sys.io.Process(cmd, args.map(function(arg) {
			return StringTools.replace(arg, '"', '\\"');
		}));
//		var r = sys.Sys.command(cmd, args);
...
//args.push("--debug-javascript");
	...
	static function tmp(ext : String) : String
	{
		...
		{
			uid = tmpuid(ext);
		} while(sys.FileSystem.exists(uid));
		return uid;
	}
