...
class thx_culture_FormatParams {
	...
	static function params($p, $ps, $alt) {
		...
		if(null === $ps || $ps->length === 0) {
			$parts = _hx_explode(":", $p);
			return _hx_deref(new _hx_array(array($parts[0])))->concat((($parts->length === 1) ? new _hx_array(array()) : thx_core_Iterators::map(_hx_explode(",", $parts[1])->iterator(), array(new _hx_lambda(array(&$alt, &$p, &$parts, &$ps), "thx_culture_FormatParams_0"), 'execute'))));
		}
		return $ps;
	}
