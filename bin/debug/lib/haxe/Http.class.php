	...
	public function customRequest($post, $api, $sock = null, $method = null) {
		...
		if($multipart) {
			...
			while($__hx__it->hasNext()) {
				...
				$uri .= _hx_string_or_null(rawurlencode($p)) . "=" . _hx_string_or_null(rawurlencode($this->params->get($p)));
			}
		}
		$b = new StringBuf();
		if($method !== null) {
		...
		$b->add(" HTTP/1.1\x0D\x0AHost: " . _hx_string_or_null($host) . "\x0D\x0A");
...
function haxe_Http_0(&$__hx__this, &$url, $status) {
	{
	}
}
...
function haxe_Http_2(&$__hx__this, &$url, $data) {
	{
	}
}
function haxe_Http_3(&$__hx__this, &$_g, &$a, &$api, &$b, &$chunked, &$headers, &$hline, &$hname, &$k, &$response, &$rp, &$s, &$size, &$sock, &$status) {
	if($a->length === 1) {
		return $a[0];
	} else {
		return $a->join(": ");
...
function haxe_Http_4(&$__hx__this, &$api, &$b, &$boundary, &$buf, &$bufsize, &$data, &$host, &$method, &$multipart, &$port, &$portString, &$post, &$request, &$secure, &$sock, &$uri, &$url_regexp) {
	if($__hx__this->file->size > $bufsize) {
		return $bufsize;
	} else {
		return $__hx__this->file->size;
	}
}
