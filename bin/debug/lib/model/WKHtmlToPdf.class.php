...
class model_WKHtmlToPdf extends model_WKHtml {
	...
	public function commandOptions() {
		$args = new _hx_array(array()); $cfg = $this->get_pdfConfig();
		if(null !== $cfg->dpi) {
			$args->push("--dpi");
			$args->push("" . _hx_string_rec($cfg->dpi, ""));
		}
		...
		if(null !== $cfg->imageDpi) {
			$args->push("--image-dpi");
			$args->push("" . _hx_string_rec($cfg->imageDpi, ""));
		}
		if(null !== $cfg->imageQuality) {
			$args->push("--image-quality");
			$args->push("" . _hx_string_rec($cfg->imageQuality, ""));
		}
		...
		if(null !== $cfg->marginTop) {
			...
			$args->push("" . _hx_string_or_null($cfg->marginTop));
		}
		if(null !== $cfg->marginBottom) {
			$args->push("--margin-bottom");
			$args->push("" . _hx_string_or_null($cfg->marginBottom));
		}
		if(null !== $cfg->marginLeft) {
			$args->push("--margin-left");
			$args->push("" . _hx_string_or_null($cfg->marginLeft));
		}
		if(null !== $cfg->marginRight) {
			$args->push("--margin-right");
			$args->push("" . _hx_string_or_null($cfg->marginRight));
		}
		...
		if(null !== $cfg->footerSpacing) {
			...
			$args->push("" . _hx_string_rec($cfg->footerSpacing, ""));
		}
		...
		if(null !== $cfg->headerSpacing) {
			...
			$args->push("" . _hx_string_rec($cfg->headerSpacing, ""));
		}
	...
	public function set_pdfConfig($c) {
		return $this->_pdfConfig = $c;
	}
	public function get_pdfConfig() {
	...
	static $__properties__ = array("set_pdfConfig" => "set_pdfConfig","get_pdfConfig" => "get_pdfConfig","set_wkConfig" => "set_wkConfig","get_wkConfig" => "get_wkConfig","set_format" => "set_format","get_format" => "get_format");
