...
class model_WKHtml {
	...
	public function renderUrl($path) {
		...
		if(!$this->execute($args)) {
			...
			haxe_Log::trace(_hx_string_or_null(model_WKHtml::cmdToString($this->cmd, $args)) . "\x0A" . _hx_string_or_null($this->cleanErr($this->err)), _hx_anonymous(array("fileName" => "WKHtml.hx", "lineNumber" => 58, "className" => "model.WKHtml", "methodName" => "renderUrl")));
		}
		if($ok) {
