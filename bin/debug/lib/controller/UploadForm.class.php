...
class controller_UploadForm extends ufront_web_mvc_Controller {
	...
	public function gist($gistid = null) {
		...
		if(null !== $id) {
			...
			return new ufront_web_mvc_ContentResult(util_Erazors::apply(new template_GistUpload(), $ob)->execute(), null);
		}
	}
	public $lastError;
	public function display($html = null, $config = null, $displayFormat = null) {
		$ob = _hx_anonymous(array("baseurl" => App::baseUrl(), "url" => new ufront_web_mvc_view_UrlHelperInst($this->controllerContext->requestContext), "html" => $html, "config" => $config, "errors" => new haxe_ds_StringMap(), "displayFormat" => $displayFormat));
		if($this->controllerContext->request->get_httpMethod() === "POST") {
			$haserrors = false;
			...
			if(null !== $config && $config !== "") {
				...
				try {
					...
					{
						$haserrors = true;
						$ob->errors->set("config", "the config file is not well formed: " . Std::string($e));
					}
				}
			}
			if(!$haserrors) {
		...
		return new ufront_web_mvc_ContentResult(util_Erazors::apply(new template_FormUpload(), $ob)->execute(), null);
	}
	...
	static $__rtti = "<class path=\"controller.UploadForm\" params=\"\" module=\"controller.UploadFormController\">\x0A\x09<extends path=\"ufront.web.mvc.Controller\"/>\x0A\x09<display public=\"1\" set=\"method\" line=\"9\"><f a=\"?html:?config:?displayFormat\">\x0A\x09<c path=\"String\"/>\x0A\x09<c path=\"String\"/>\x0A\x09<c path=\"String\"/>\x0A\x09<d/>\x0A</f></display>\x0A\x09<lastError><c path=\"String\"/></lastError>\x0A\x09<gist public=\"1\" set=\"method\" line=\"60\"><f a=\"?gistid\">\x0A\x09<c path=\"String\"/>\x0A\x09<c path=\"ufront.web.mvc.ActionResult\"/>\x0A</f></gist>\x0A\x09<validateGist set=\"method\" line=\"79\"><f a=\"id\">\x0A\x09<c path=\"String\"/>\x0A\x09<c path=\"String\"/>\x0A</f></validateGist>\x0A\x09<new public=\"1\" set=\"method\" line=\"7\"><f a=\"\"><x path=\"Void\"/></f></new>\x0A</class>";
