...
class thx_date_Milli {
	...
	static function toString($v, $shortFormat = null) {
		if($shortFormat === null) {
			$shortFormat = true;
		}
		...
		while($cur < $len && $v >= 1) {
			...
			if($div > 0) {
				if($shortFormat) {
					$s->push(_hx_string_rec($div, "") . _hx_string_or_null($item->short));
				} else {
					$s->push(_hx_string_rec($div, "") . " " . _hx_string_or_null($item->name) . _hx_string_or_null(((($div === 1) ? "" : "s"))));
				}
				$v = $v - $div * $item->time;
			}
			$cur++;
...
thx_date_Milli::$map = thx_date_Milli_0();
function thx_date_Milli_0() {
	{
		$m = new haxe_ds_StringMap();
		thx_core_Arrays::each(thx_date_Milli::$intervals, array(new _hx_lambda(array(&$m), "thx_date_Milli_1"), 'execute'));
		return $m;
	}
}
