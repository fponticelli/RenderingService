...
class ufront_web_PartialUrl {
	...
	public function queryString() {
		...
		while($__hx__it->hasNext()) {
			...
			$params->push(_hx_string_or_null($param) . "=" . _hx_string_or_null((ufront_web_PartialUrl_0($this, $item, $param, $params))));
			unset($item);
		}
		return $params->join("&");
	}
...
function ufront_web_PartialUrl_0(&$__hx__this, &$item, &$param, &$params) {
	if($item->encoded) {
		return $item->value;
	} else {
