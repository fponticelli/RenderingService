...
class ufront_web_mvc_ControllerActionInvoker implements ufront_web_mvc_IActionInvoker{
	...
	public function getAttributeClass($className) {
		...
		$__hx__it = $this->controllerBuilder->attributes->iterator();
		while($__hx__it->hasNext()) {
			$pack = $__hx__it->next();
			$c = Type::resolveClass(_hx_string_or_null($pack) . "." . _hx_string_or_null((ufront_web_mvc_ControllerActionInvoker_0($this, $className, $pack))) . "Attribute");
	...
	public function createClassTree($cls, $array = null) {
		if($array === null) {
	...
	public function getAttributes($controller, $actionField) {
		...
		$metadata = Lambda::map($classes, array(new _hx_lambda(array(&$actionField, &$classes, &$controller), "ufront_web_mvc_ControllerActionInvoker_1"), 'execute'));
		$metadata->add($this->getFieldAttributes($controller, $actionField));
		$hash = Lambda::fold($metadata, array(new _hx_lambda(array(&$actionField, &$classes, &$controller, &$metadata), "ufront_web_mvc_ControllerActionInvoker_2"), 'execute'), new haxe_ds_StringMap());
		$self = $this;
		$objects = Lambda::map(_hx_anonymous(array("iterator" => ufront_web_mvc_ControllerActionInvoker_3($this, $actionField, $classes, $controller, $hash, $metadata, $self))), array(new _hx_lambda(array(&$actionField, &$classes, &$controller, &$hash, &$metadata, &$self), "ufront_web_mvc_ControllerActionInvoker_4"), 'execute'));
	...
	public function getFilters($context, $actionField) {
		...
		$attributes->sort(array(new _hx_lambda(array(&$actionField, &$attributes, &$context), "ufront_web_mvc_ControllerActionInvoker_6"), 'execute'));
	...
	public function invokeAction($controllerContext, $actionName, $async) {
		...
		if(ufront_web_mvc_ControllerActionInvoker::$_mapper->exists($actionName)) {
			$action = "h" . _hx_string_or_null($actionName);
		}
		...
		try {
			...
			if(null !== $authorizationContext->result) {
				...
				if(null !== $executingContext->result || !$isasync) {
					...
					$me = $this;
					$handler = array(new _hx_lambda(array(&$action, &$actionName, &$arguments, &$async, &$authorizationContext, &$cls, &$controller, &$controllerContext, &$e, &$executingContext, &$fields, &$filterInfo, &$isasync, &$me, &$method), "ufront_web_mvc_ControllerActionInvoker_7"), 'execute');
			...
			$e2 = $_ex_;
			{
				...
				ufront_web_mvc_ControllerActionInvoker::createActionResult($exceptionContext->get_result())->executeResult($controllerContext);
			}
		}
	}
	...
	public function getParameterValue($controllerContext, $parameter) {
		...
		$bindingContext = new ufront_web_mvc_ModelBindingContext($parameter->name, $parameter->type, $controllerContext->controller->get_valueProvider(), $parameter->ctype);
	...
	static function isAsync($method) {
		...
		$last = $arguments->pop();
		return ufront_web_mvc_ControllerActionInvoker_8($arguments, $last, $method);
	}
...
function ufront_web_mvc_ControllerActionInvoker_0(&$__hx__this, &$className, &$pack) {
	if($className === null) {
		return null;
	} else {
		return _hx_string_or_null(strtoupper(_hx_char_at($className, 0))) . _hx_string_or_null(_hx_substr($className, 1, null));
	}
}
function ufront_web_mvc_ControllerActionInvoker_1(&$actionField, &$classes, &$controller, $c) {
	{
		return haxe_rtti_Meta::getType($c);
	}
}
function ufront_web_mvc_ControllerActionInvoker_2(&$actionField, &$classes, &$controller, &$metadata, $meta, $output) {
	{
		if($meta === null) {
			return $output;
		}
...
function ufront_web_mvc_ControllerActionInvoker_3(&$__hx__this, &$actionField, &$classes, &$controller, &$hash, &$metadata, &$self) {
	{
		$_e = $hash;
		return array(new _hx_lambda(array(&$_e, &$actionField, &$classes, &$controller, &$hash, &$metadata, &$self), "ufront_web_mvc_ControllerActionInvoker_9"), 'execute');
	}
}
		...
		$args = $hash->get($key);
		{
			...
			while($_g < $_g1->length) {
				...
				if(!_hx_has_field($instance, $arg)) {
					throw new HException(new thx_error_Error("Filter " . _hx_string_or_null(Type::getClassName(Type::getClass($instance))) . " has no field " . _hx_string_or_null($arg), null, null, _hx_anonymous(array("fileName" => "ControllerActionInvoker.hx", "lineNumber" => 294, "className" => "ufront.web.mvc.ControllerActionInvoker", "methodName" => "getAttributes"))));
				}
...
function ufront_web_mvc_ControllerActionInvoker_6(&$actionField, &$attributes, &$context, $x, $y) {
	{
		return $x->order - $y->order;
	}
}
function ufront_web_mvc_ControllerActionInvoker_7(&$action, &$actionName, &$arguments, &$async, &$authorizationContext, &$cls, &$controller, &$controllerContext, &$e, &$executingContext, &$fields, &$filterInfo, &$isasync, &$me, &$method, $value) {
	{
...
function ufront_web_mvc_ControllerActionInvoker_8(&$arguments, &$last, &$method) {
	$__hx__t = ($last->t);
	switch($__hx__t->index) {
	case 4:
	$ret = $__hx__t->params[1]; $args = $__hx__t->params[0];
	{
		if($args->length !== 1) {
			false;
		}
		...
		case 1:
		$ret_eCEnum_1 = $__hx__t2->params[1]; $t = $__hx__t2->params[0];
		{
			return $t === "Void";
		}break;
		default:{
...
function ufront_web_mvc_ControllerActionInvoker_9(&$_e, &$actionField, &$classes, &$controller, &$hash, &$metadata, &$self) {
	{
		return $_e->keys();
	}
}
