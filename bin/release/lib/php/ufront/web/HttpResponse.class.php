...
class php_ufront_web_HttpResponse extends ufront_web_HttpResponse {
	...
	public function flush() {
		...
		header("HTTP/1.1 " . _hx_string_or_null(php_ufront_web_HttpResponse::statusDescription($this->status)), true, $this->status);
		try {
			$__hx__it = call_user_func((php_ufront_web_HttpResponse_0($this, $cookie, $k, $v)));
			while($__hx__it->hasNext()) {
				$cookie = $__hx__it->next();
				$expire = php_ufront_web_HttpResponse_1($this, $cookie, $k, $v);
		...
		try {
			...
			while($__hx__it->hasNext()) {
				...
				header(php_ufront_web_HttpResponse_2($this, $e, $k, $key, $v), true);
			}
		}catch(Exception $__hx__e) {
			$_ex_ = ($__hx__e instanceof HException) ? $__hx__e->e : $__hx__e;
			$e2 = $_ex_;
...
function php_ufront_web_HttpResponse_0(&$__hx__this, &$cookie, &$k, &$v) {
	{
		$_e = $__hx__this->_cookies;
		return array(new _hx_lambda(array(&$_e, &$cookie, &$k, &$v), "php_ufront_web_HttpResponse_3"), 'execute');
	}
}
function php_ufront_web_HttpResponse_1(&$__hx__this, &$cookie, &$k, &$v) {
...
function php_ufront_web_HttpResponse_2(&$__hx__this, &$e, &$k, &$key, &$v) {
	if($v === null) {
		return $key;
	} else {
...
function php_ufront_web_HttpResponse_3(&$_e, &$cookie, &$k, &$v) {
	{
		return $_e->iterator();
	}
}
