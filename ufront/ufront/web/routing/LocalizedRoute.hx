...
class LocalizedRoute extends Route
{
	...
	public function new(translator : ITranslation, url : String, handler : IRouteHandler, ?defaults : Map<String, String>, ?constraints : Array<IRouteConstraint>)
	{
	...
	var _asts : Map<String, UriSegments>;
 ...
 //   	trace(httpContext.request.query);
	...
	override function getPath(httpContext : HttpContext, data : Map<String, String>)
	{
	...
	public static function addLocalizedRoute(collection : RouteCollection, translator : ITranslation, uri : String, ?defaults : Dynamic<String>, ?constraints : Array<IRouteConstraint>)
	{
		collection.add(
	   		new LocalizedRoute(
		    	...
		    	uri,
				new MvcRouteHandler(),
				null == defaults ? null : defaults.toMap(),
				constraints));
	}
}