...
class Acl
{
	...
	var _rules : {
		allResources : {
			allRoles : {
				allPrivileges : Combo,
				byPrivilegeId : Map<String, Combo>
			},
			byRoleId : Map<String, { byPrivilegeId : Map<String, Combo>, allPrivileges : Combo }>
		},
		byResourceId : Map<String, {
			allRoles : {
				allPrivileges : Combo,
				byPrivilegeId : Map<String, Combo>
			},
	...
	function _getRules(resource : String, role : String, create = false)
	{
		...
		if(null == resource)
		{
			...
			if(!_rules.byResourceId.exists(resource))
			{
				...
				_rules.byResourceId.set(resource,
					{
						byRoleId : new Map<String, { byPrivilegeId : Map<String, Combo>, allPrivileges : Combo }>(),
						allRoles : {
							allPrivileges : null,
							byPrivilegeId : new Map()
						}
		...
		if(!visitor.byRoleId.exists(role))
		{
			...
			visitor.byRoleId.set(role, {
				byPrivilegeId : new Map<String, Combo>(),
				allPrivileges : null
			});
		}
